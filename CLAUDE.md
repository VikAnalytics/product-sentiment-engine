...
| `check_pipeline_health.py` | Fail the workflow when a run degraded, or when no run happened at all. `--heartbeat` |
...
- **Paged queries must be ordered**: `fetch_all_rows` on an unordered query is unsafe. Postgres may order each page differently, so rows get skipped or duplicated and identical runs disagree. This made the first backtest results vary by 13 percentage points between runs. Always `.order("id")` unless another key is needed.
- **Replay needs an injectable clock**: `_score_candidates(..., as_of=)` exists because sentiment momentum compares a recent window to the one before it. Scoring historical data against `datetime.now()` puts every reading outside both windows, momentum reads zero for the whole universe, and the regime filter then calls every day risk-off.
- **Retired runs are summarised in `sim_runs`** (migration 022): one row per reset with the final value and the SPY/QQQ values over that run's own window. A run's last fortnightly snapshot can be weeks before it actually ended, so without this the previous run's ending was unknowable once the portfolio row was overwritten. The run retired on 2026-09-19 is recorded at its last active valuation instead: $1,117.31 on 2026-06-22 (+11.73%) against SPY +8.77% and QQQ +19.66%. It stopped trading on 2026-06-12, so marking its final position to a September close would charge it three months of drift it took no part in. `scripts/record_previous_run.py` writes that row, since the reset predated the table.
- **Resetting the simulator keeps the record**: `--action reset` (with `--dry-run`) returns cash and peak to the starting capital and sets `initialized_at` to today, which also rebases the SPY/QQQ comparison. It deletes nothing: open positions are closed into `sim_trades` at the last known price, because a position that simply vanished would leave a buy with no sell and overstate past performance. Anything reading `sim_trades` or `sim_snapshots` must filter to `>= sim_portfolio.initialized_at` or it will mix runs. Reset on 2026-09-19; the prior run ended at $1,099.12.
...
- **A run that reports success is not a run that worked**: the engine wrote nothing from 2026-06-10 while every workflow exited 0, because OpenAI answered 429 to each extraction call and the failure only reached a log nobody read. `scripts/check_pipeline_health.py` now ends the daily workflow: any step `failed`, `degraded`, left `running`, or missing from `pipeline_runs` fails the job, so GitHub sends its own mail and the reason lands in the run summary. `assess()` is pure and unit-tested, because the cases that matter — a degraded step, a step that never ran, a retry that supersedes an earlier failure — are the ones hardest to reproduce against a live database.
- **A watchdog cannot live inside the thing it watches**: the trigger stopped on 2026-07-04 and nothing ran for eleven weeks, which no in-run check can report because no run happens to report it. `.github/workflows/pipeline_heartbeat.yml` runs on GitHub's own schedule at 02:00 UTC and fails when `pipeline_runs` holds no successful step in 26 hours. It installs `supabase` and `python-dotenv` only — it reads one table and has no reason to wait for spacy and torch. Note that GitHub disables scheduled workflows after 60 days without repository activity.
